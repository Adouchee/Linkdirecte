...
- `identifiant`: The user's username.
- `motdepasse`: The user's password.
- `options`: (Optional) Configuration for the login process.
  - `rememberMe`: If true, attempts to store an access token, UUID and 2FA tokens (`cn`/`cv`) for future refreshes.
    - `on2faRequired`: (Optional) A callback function to handle 2FA automatically. It receives the question and choices, and should return the index of the chosen answer.
...
- **`LoginChallenge`**: If 2FA is required.
  ...
  - `answer(index)`: A function to submit the chosen answer. Returns a `Promise` of the next step (usually `LoginSuccess`).
...
if (result.type === "securityQuestion") {
  const session = await result.answer(0);
}
